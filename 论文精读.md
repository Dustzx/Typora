...
### 9. A Survey on Transformers in Reinforcement Learning

<a name='md9'>-</a>[原文](papers/.pdf)
...
Transformer架构在NLP和CV领域已经成了最为流行的架构，如今这一趋势同样出现在了RL领域。由于RL本身的特性，对于二者的适配因此存在一些问题，并且目前仍未被很好地解决。该文即系统回顾RL中对于Transformer的应用，对其进行分类并提出发展方向。
...
本文具体将RL中的Transformer分为了

- Transformers作为RL的模型(表示模型/世界模型etc.)
- Transformers作为序列决策者(sequential decision-maker)
- Transformers有利于跨任务、跨模态的泛化性

下面是本文对于未来发展的观点和该方向的开放性问题

###### Combining Reinforcement Learning and (Self-) Supervised Learning

当使用Transformer通过序列建模去解决决策问题时，deadly triad problem在监督学习的范式下得到了缓解。
在该范式下，学习到的策略非常依赖于offline-data的质量，并且在exploitation和exploration之间的trade-off不再存在。因此通过结合RL和监督学习可以通过Transformer可能学习到更好的策略。但是该情况也会导致exploration会受限于相对固定的策略。

### 10. Symptoms are known by their companies

<a name='md10'>-</a>[原文](papers/Symptoms are known by their companies.pdf)

①期刊会议+标题+作者

2022 BMC Bioinformatics——CCF-C

Symptoms are known by their companies: towards association guided disease diagnosis assistant

症状与症状之间互有联系，症状联系导向的疾病诊断助手	
...
开始先说套话，讲医疗领域的需求难以满足，AI能助力解决该问题。

一个症状可以推及到其他一些症状的存在与否，并且他们之间的联系为病人当前所处的状况提供了重要信息

> one symptom may strongly suggest another symptom’s existence/non-existence, and their association provides crucial information about the suffering condition. 

本文提出并构建了一个名为Association guided Symptom Investigation and Diagnosis Assistant (A-SIDA)的虚拟助手。(基于**症状相关性**的症状询问和疾病诊断助手。)

提了一嘴使用的是分层级的强化学习。

又讲A-SIDA怎么工作，并介绍其融合了association-based recommendations 和 critic。(第一次见带critic的模型)

本文提出了一个问询相关性分数的评价指标 Investigation Relevance Score (IReS)，该指标用来评测问询到的症状的相关性。最后展示了自己哪些指标得到了提升，并强调了其中提升最大的指标，解释了提升的原因。

③结论

又讲一遍套话，公共医疗系统满足不了公众需求，AI来当救世主。

之后又是讲本文核心的灵感，跟摘要相比换个说法。

对于模型的实现，展开讲了分层级的强化学习具体分为了两级，一级是 an association & recommendation module (ARM)，另一级是 a disease classifier。

之后又细讲两级的作用，以及其构成。

其中ARM通过使用上下文对齐的联系导向的症状问询和一个额外的critic来增强assistant。（即核心创新点的作用）
另一级就是基于self-report和额外问询到的症状的正常诊断。

Furthermore，又提另一个创新点，Investigation relevance score（IReS）——新的评测标准(**若当前工作，在优化目标上有所欠缺时，可以引入新的评价标准作为创新点**)

临结束与其他模型进行比较，相较于摘要没有引用数据，泛泛地说比哪些要好，并且达到了SOTA。泛泛讲完后，提了自己的特色——加上了人工评估(该项分数高也对应自己的创新点，再次证明自己核心创新点的重要性)。

最后讲了未来的工作打算，想要将症状询问和疾病诊断统一为一个网络结构去完成。(Diaformer实现了，该种一个模型完成)
并且还想研究多模态诊断。
...
此处显式说明诊断过程中分析症状是其中的重要步骤，也就区别于以前模型只进行症状问询和疾病诊断两大步，使得核心创新点症状关联程度的重要性得以凸显。（<u>作者此处并没有刻意解释和强调分析症状的重要性，只说了有三个步骤</u>）

第一段剩余内容为套话，分析现状：

> As reported by the World health organization (WHO), 2013 [1], the world falls short of 7.2 millions medical workers, which is expected to reach 12.9 millions in the upcoming decade. The dearth still continues as per a report by WHO, 2019 [2], there are many countries where doctor per 1000 people is less than one. These figures firmly suggest the betterment of the healthcare system by increasing health workers and utilizing their time more efficiently and critically. As a result, there is a surge of interest in utilizing Artificial Intelligence (AI) based systems to reduce the workload of medical professionals [3]. One of such manifestations is automatic diagnosis with the help of a virtual agent that can conduct a thorough symptom investigation and present a detailed report to doctors.
> Some automatic disease diagnosis systems such as Mayo Clinic 1, Babylon Healthcare 2 and GMAN are already deployed, which are being extensively used by both hospitals and end-users. A study conducted by Fox et al. [4] showed 35% U.S adults had utilized self-diagnosing tools before consulting with real doctors.

就最后一句 A typical diagnosis process has been illustrated in Fig. 1.不是废话。当然上面套话的角度需要学习。国际数据显示的需求+目前AI存在的潜力+已经部署的某些工业界产品。

用统计症状空间大小的方式来解释核心点的重要性（不同于摘要与结论）。之后又讲了一些看似与核心点有关的东西，并且这些东西反应了该方向数据集的问题（<u>mz10中似乎解决了</u>）。

> In real world, a considerably large number of diseases are diagnosed by doctors through only an in-depth symptom investigation [7]. In some cases, they need further evidence through laboratory reports to reach a conclusive diagnosis. Nevertheless, symptom investigation is essential and crucial for suggesting an appropriate lab examination.

下一段开始介绍相关工作，叙述其他工作的核心点后说明他们都没有关注本文的核心点，并且再次强调本文核心点的重要性，最后说：

> Motivated by the research gap, we aim to investigate the efficacy of symptom association in disease diagnosis and build a symptom association-guided disease diagnosis assistant.

介绍之前同摘要一样，对核心概念来了一个类比，引用The role of friends’ appearance and behavior on evaluations of individuals on facebook: are we known by the company we keep?。同样又引用Learning words from context和Efficient estimation of word representations in vector space。类比后再次解释自己核心点的重要性，并强调该创新点以前在本领域没有人用过。

下段引用其他论文的观点

> The primary objectives of any autonomous disease diagnosis system are to diagnose
> patients accurately and efficiently
>
> ——Dialogue based disease screening through domain customized reinforcement learning.

之后关于症状相关性实际作用的论述中，似乎太过偏重于用户体验，而不是从医生诊断逻辑/经验角度去阐述。

强调完创新点实际作用后再说以往工作没有重视这一点，最后将所有的创新点进行总结。创新点：

- 工作统计了诊断过程中，症状之间的联系，提出了基于HRL的新的症状联系导向的问询方法
- 提出了一种新的评价准则Investigation Relevance Score(IReS)，去评测问询的症状之间的相关性
- 模型性能达到了SOTA

⑥模型

两个图，第一个图描述自动诊断对话系统的工作流程；第二个图描述模型的架构。

之后分开讲模型的各个模块

- Symptom investigation
  - Diagnosis Policy Learning
  - Association & relevance module (ARM)
  - Internal & Association and Recommendation (AR) critic
- Diagnosis state tracker, patient and disease classifier
- Investigation relevance score (IReS)

使用别人的方法时：

> Motivated by the real-world scenario and the promising results obtained by Liao et al. [12, 37], we also utilized a hierarchical policy learning method,where the ……

核心的Association & relevance module (ARM)的作用是进行基于知识的症状问询，ARM的输入是the current state ($S_t$) and inspected symptom ($Sym_t$ )，输出为association score & symptom recommendation ($RS_t$ )，其中关联度分数为所问询的症状与所有历史症状之间关联度求和。
$$
as_t=\sum^{n_t}_{k=1}Association(Sym_t,SS_k)
\tag{1}
$$
关联度计算方式：
$$
Association(S_i,S_j) = \frac{n(S_i,S_j)}{\sum_{k}n(S_i,S_k)}
\tag{2}
$$
之后介绍了每个参数的含义

> where n(Si, Sj) is the number of instances in the diagnosis dataset, where S i and S j have co-occurred.
>
> etc.

介绍完关联度分数，介绍ARM中症状推荐部分，展开具体内容前再次强调了motivation，此处需借鉴

> A symptom may strongly suggest the existence of another symptom, which are caused
> by a common condition. For instance, when we think about cold, the next symptom that comes to our mind is cough. Cold and cough often co-occur together. Motivated by the observation, the proposed model incorporates a recommendation module

推荐模块使用关联度分数去选了top K个症状
$$
RS=\prod^{K}_{i=1}argmax_{s∈S_y}\sum^{|SS|}{j=1}(SS_j,s)
\tag{3}
$$
...
> The work [34] presents a low cost millimeter antenna for building portable 5G communication gadgets. 

混进来一个无关的凑字数

a knowledge-infused contextdriven (KI-CD) disease diagnosis model一文中的idea是

> In real life, doctors continuously exploits extracted symptom information for intelligent symptom inspection. They hypothesize a set of probable diseases based on extracted symptoms and first inspects the potential symptoms of these candidate diseases. 

此处写作手法是先讲该idea再说Tiwari et al. [5] investigated the idea and proposed  KI-CD

⑧评论

### 11. End-to-End Knowledge-Routed Relational Dialogue System for Automatic Diagnosis

<a name='md11'>-</a>[原文](papers/s12859-022-05032-y.pdf)
...
End-to-End Knowledge-Routed Relational Dialogue System for Automatic Diagnosis
 ...
 用于自动诊断的端到端<u>知识路由关系对话系统</u> 

中山大学

②摘要

由于是早期工作，故开篇对于自动化诊断与常规的对话系统进行了区分。提出自动化诊断是患者自报告症状-通过对话收集症状-诊断。强调了该方向的对话需要在医学相关知识和症状-疾病关系的背景下更具合理性（对话中应该有严谨的医学逻辑）。

本文的贡献是：

- 提出了端到端的知识路由关系对话系统，End-to-End Knowledge-routed Relational Dialogue System (KR-DS)，将丰富的医学知识图融入到了对话管理中的话题转变(症状询问->疾病诊断)
- 提出了一个基于知识路由的DQN，Knowledge-routed Deep Q-network (KR-DQN)。其包含1）关系提炼分支编码症状间、症状-疾病间关系 2）知识路由的图分支，用来选择话题。(KR-DS的一部分)
- 收集了新的数据集，该数据集保留了更为原始的患者自报告和医生-患者对话数据

③结论

解释了End to End，即End-to-End Knowledge-routed Relational Dialogue System (KR-DS) 可以使得对话管理、自然语言理解和自然语言生成在强化学习的范式下协作进行优化。

④导言

先讲任务导向对话系统发展近况，又讲作为其分支的自动诊断医疗对话系统的重要性。

> which has significant potential to simplify the diagnostic procedure and reduce the cost of collecting information from patients (Tang et al. 2016). Besides, patient condition reports and preliminary diagnosis reports generated by the dialogue system may assist doctors to make a diagnosis more efficiently.

之后引出自动化诊断医疗对话系统所面临问题：医学诊断对话系统不仅对医学知识背景下的对话合理性提出了严格的要求，而且对症状-疾病关系的理解也提出了严格的要求。

所询问的症状需要与疾病和医学知识有潜在联系。

> The symptoms which dialogue system inquiries should be related with underlying disease and consistent with medical knowledge

![1684912306130](论文精读.assets/1684912306130.png)

之后由上图可以看出，之前的对话系统生成的结果复杂且重复。并且该baseline仅在对话管理部分（对话状态追踪、策略学习），其余部分都是直接使用模板，也就使得效果不好。

之后引出自己的创新KR-DQN，好在哪里：
两个分支knowledge-routed graph branch、relational refinement branch可以分别获取医学知识和医生的诊断经验，这是医生在进行诊断时所依托的两个核心要素。

- The relational refinement branch自动化学习symptom-symptom和symptom-disease对之间的关系，去优化基础DQN生成的结果。
- knowledge-routed graph branch通过精心设计的基于条件概率的医学知识图路由先验信息来帮助决策 

又由于Baseline中提出的数据集不包含原始对话，无法支撑本文模型进行端到端训练，本文又构建了新的对话数据集。



⑥模型

KR-DS同传统TOD一样分三部分，NLU、DM和NLG。其中DM负责依据当前的对话状态进行话题的转变(症状询问->疾病诊断)，DM中的agent负责进行问诊或者诊断。对于生成的系统动作(症状/疾病)，交由基于模板的NLG去生成输出。为实现end2end训练，又添加了拥友真实数据的用户模拟器与模型进行交互。

**NLU**

主要用来进行意图分类和槽填充，给定一句话，NLU分类出这句话的意图，并将其中重要的内容作为槽值填充到槽中。该部分使用的模型是Bi-LSTM.
...
![1684919595526](论文精读.assets/1684919595526.png)

从图中可以看出输入同时进入Basic-DQN和KR-DQN，并且Basic-DQN的输出经由Relational Refinement Branch进行了处理

其中最重要的**Knowledge-routed Graph Branch**，作者设计的理由是，当医生拿到患者的自我报告时，会对其所患的疾病有的大致的范围，之后随着医生询问一些症状并得到反馈后，会排除掉其中的部分疾病，知道最终确定某一种疾病。KR中存储了疾病和症状间的条件概率，边只存在疾病和症状之间，并且每个边有两个权重，分别是$P(dis|sym) ∈ R^{M×N}$和$P(sym|dis) ∈ R^{N×M}$，即疾病与症状相互之间的概率。该部分输出的动作空间概率(疾病+症状+Greeting actions)中$P(dis) = P(dis|sym)·P_{prior}(sym).$    $P(sym) = P(sym|dis)·P(dis).$ 其中的$P_{prior}(sym)∈ R^N$的计算规则：

> For the mentioned symptoms, positive symptoms is set to 1 while negative symptoms is set to -1 to discourage its related diseases. Other symptom(not sure or not mention) probabilities are set to its prior probabilities which are calculated from the dataset. 

最终的输出由以上三部分动作空间概率相加：
$a_t = sigmoid(a^{r}_t ) + sigmoid(a^{f}_t  ) + a^{k}_t $

此外为了防止生成重复信息，对于KR-DQN的输出又增加了症状过滤器。
